...
# VirtualMachine类，管理最高层的结构，特别是调用栈，同时管理指令到操作的映射，是最开始写的Interpreter类的高级版本。
# Block类，一个只包装了三个属性的类，控制代码流程的时候会用到。
...
class VirtualMachine(object):
    ...
    def  unwind_block(self,block):
        ...
        if block.type == 'execpt-handler':
            traceback,value,exectype = self.popn(3)
            self.last_exception = exectype,value,traceback

    def manage_block_stack(self,why):
        # 管理一个frame的block栈，在循环，异常处理，返回这几个方面操作与block栈与数据栈
        frame = self.frame
        ...
        if (block.type in ['setup-execept','finally'] and why == 'exception'):
            self.push_block('execpt-handler')
            exctype,value,tb = self.last_exception
            self.push(tb,value,exctype)
            self.push(tb,value,exectype)
            self.push(tb,value,exctype)
            why = None
    ...
    # 新建一个帧，code是code_obj,callargs是函数调用时的参数
...
# 运行frame
    def run_frame(self,frame):
        ...
        while True:
            ...
            while why and frame.block_stack:
                why = self.manage_block_stock(why)
            if why:
                break
    ...
    def parse_byte_and_args(self):
        ...
        # 指令码 <dis.HAVE_ARGUMENT>的都是无参的指令。其他都是有参数指令
...
class Function(object):
    ...
    __slots__ = ['func_code','fund_name','func_defaults','func_globals','func_locals','func_dict','func_closures','__name__','__dict__','__doc__','_vm','_func']
