...
# 1-简单压缩： 根据字符串出现的字符，将ASCII替换成更短的表示形式 
# 2-霍夫曼压缩： 根据字符串出现频率，构建Trie树， 对每个tree node进行定义，使得频率越高的字符离root节点越近
...
class HuffmanCompression:
    ...
    def bfs_encode(self,root,s2b):
        ...
        while queue:
            ...
            if node.right:
                node.right.coding = node.coding + '0'
...
if __name__ == '__main__':
    ...
    print(sc.s2b)
    print('Simple Compression-comress rate:%d' % get_rate(compressed, bits))    
    print('-*-'*10)
    # huffman compression
    hc = HuffmanCompression(s)
    compressed = hc.compress()  
    print ('Copressed binary:' + compressed)
